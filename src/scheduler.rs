...
impl Scheduler {
    ...
    pub fn get_global_luastate(&self) -> uintptr_t {
        #[cfg(target_pointer_width = "32")]
        type RbxGetStateT = extern "thiscall" fn(ctx: uintptr_t, out: *const i32) -> uintptr_t;

        #[cfg(target_pointer_width = "64")]
        type RbxGetStateT = extern "C" fn(ctx: uintptr_t, out: *const i32) -> uintptr_t;

        unsafe {
            let state = 0;
            (API.get_state)(self.script_context, &state)
        }
    }
